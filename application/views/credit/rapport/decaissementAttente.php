...
<div class="x_panel tile " id="tet" style="display:none;">
  ...
  <div class="x_content">
 <!--     
 <div id="impre" class="container" title="" style="margin-top: 1%;display:none;">
  <div class="card-header col-8 place">
    <h5 align="center" class="titre">Rapport Décaissement en attente</h5>
  </div>
  <br/>
    ...
    <div class="modal-dialog" style="margin-top: 80px;" role="document">
      <div class="modal-content"> 
        <div class="modal-header">
          <h5 class="modal-title" id="titre">Décaissement en attente</h5>
        </div>
           ...
           <form id="myForm" action="" method="post">
              ...
              <div class="col-12">
                ...
                <div class="form-group row">
                    <label for="dateDeb" class="col-sm-4">Date rapport : <span id="champObligatoir">*</span></label>
                    <div class="col-sm-6">
...
function getDonne(data){
      $.ajax({
            type:'ajax',
            method: "post",
            url: '<?php echo base_url();?>index.php/getDecaissementAttente',
            dataType:'json',
            data: data,
            async: false,
                 ...
                 // var detail = response.cred;
                  ...
                  if(c.length == null){
                    if (c.individuel != null) {
                      nom = c.individuel.codeInd +' |'+
                         c.individuel.nomClient +' '+c.individuel.prenomClient; 
                    }else if(c.groupe != null){
                       nom = c.groupe.codeGrp +' |'+
                         c.groupe.nomGroupe;
                    }
                      html += '<tr>' +
                        '<td>' + c.numCredit + '</td>' +
                    ...
                    for (i = 0; i < c.length; i++) {
                       if (c[i].individuel != null) {
                          nom = c[i].individuel.codeInd +' |'+
                             c[i].individuel.nomClient +' '+c[i].individuel.prenomClient; 
                        }else if(c[i].groupe != null){
                           nom = c[i].groupe.codeGrp +' |'+c[i].groupe.nomGroupe;
                        }
                        html += '<tr>' +
                        '<td>' + c[i].numCredit + '</td>' +
                        '<td>' +nom+ '</td>' +
